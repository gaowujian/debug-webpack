// ! webpack 的默认配置，也就是说只创建了一个空的webpack.config.js文件的时候，webpack进行打包会默认生成的compiler配置
/*
...
class WebpackOptionsDefaulter extends OptionsDefaulter {
  constructor() {
    ...
    this.set("optimization.minimizer", "make", (options) => [
      {
        apply: (compiler) => {
          ...
          new TerserPlugin({
            ...
            parallel: true,
            sourceMap:
              (options.devtool && /source-?map/.test(options.devtool)) || (options.plugins && options.plugins.some((p) => p instanceof SourceMapDevToolPlugin)),
          }).apply(compiler);
        },
      },
    ]);
    ...
    this.set("resolve.preferAbsolute", "make", (options) => !options.resolve.roots || options.resolve.roots.length === 0);
    this.set("resolve.ignoreRootsErrors", "make", (options) => !options.resolve.roots || options.resolve.roots.length === 0);
